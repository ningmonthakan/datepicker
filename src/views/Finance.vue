                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="text-center">
                      ...
                      <v-btn
                        :disabled = "!item.check_approvement_attached"
                        small
                        @click="showAlert('ยืนยันหลักฐาน',item.document_uid)"
                        class="ma-2"
                        color="primary"
                      >
                        ยืนยันหลักฐาน
...
import DownloadDialog from '../components/DownloadDialog.vue'
  ...
  export default {
    ...
    created() {
      ...
      if (this.role.includes('ALL')||this.role.includes('FINANCE')) {
        this.getdata ()
        this.getdata_amount ()
      }else {
    ...
    methods:{
      ...
      getdata_amount () {
        ...
        this.axios.get(process.env.VUE_APP_API + '/dashboard/document/amount', headers).then((response) => {
      ...
      getdata () {
        ...
        this.axios.get(process.env.VUE_APP_API + '/dashboard/document?page=' + this.page + '&status=' + this.status + '&search=' + this.search, headers).then((response) => {
          this.loading = false
      ...
      put_verify (status,remark,uid) {
        ...
        this.axios.put(process.env.VUE_APP_API + '/dashboard/document/verify/' + uid, data, headers)
        .then(() => {
          this.$swal.fire({
              title: "สำเร็จ",
              icon: 'success',
      ...
      put_attached (status,remark,uid) {
        ...
        this.axios.put(process.env.VUE_APP_API + '/dashboard/document/attach/' + uid, data, headers)
        .then(() => {
          this.$swal.fire({
              title: "สำเร็จ",
              icon: 'success',
      ...
      put_status (status,remark,uid) {
        ...
        this.axios.put(process.env.VUE_APP_API + '/dashboard/document/status/' + uid, data, headers)
        .then(() => {
          this.$swal.fire({
              title: "สำเร็จ",
              icon: 'success',
