      ...
      <div>
        <v-row>
          <v-col
            ...
            "
          >
            <h3 style="font-weight:700">ใบแจ้งหนี้</h3>
          </v-col>
          <v-col cols="3">
          ...
          <v-col cols="12">
            <v-card>
              ...
              <v-card-text>
                <!-- <download-csv :data="debt_data" name="export_billing_all.csv"
                  ><v-btn
                    color="teal"
                    dark
...
export default {
  ...
  methods: {
    ...
    exportAll(){
      ...
      this.axios
        .get(process.env.VUE_APP_API + "/export/log/billing",headers)
        .then((response) => {
            if(response.status == 200) {
              const fileURL = window.URL.createObjectURL(response.data);
              const fileLink = document.createElement("a");
              fileLink.href = fileURL;
              fileLink.setAttribute("download", "export_request_billing_all.csv");
