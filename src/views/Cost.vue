<template>
  <v-container>
      ...
      <v-col cols="12">
        <v-card>
          ...
          <v-card-text>
            ...
            >
              <template v-if="data.length != 0" v-slot:body="{ items }">
                <tbody>
                  <tr v-for="item in items" :key="item.doc_no">
                    ...
                    <td class="text-center">
                      ...
                      <v-btn
                        :disabled="!item.check_approvement"
                        small
                        @click="select_transfer(item.document_uid)"
                        class="ma-2"
                        color="primary"
                      >
                        ย้ายกลุ่มตัดงบ
    ...
    <download-dialog
      ...
      @close_download_dialog="close_download_dialog"
    />
  </v-container>
</template>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "รหัสพนักงาน", align: "center", sortable: false, width: 150 },
      { text: "ชื่อ-นามสกุล", align: "start", sortable: false, width: 200 },
      { text: "จำนวนเงิน", align: "center", sortable: false, width: 150 },
      { text: "สถานะเอกสาร", align: "center", sortable: false, width: 150 },
      { text: "สถานะตัดงบ", align: "center", sortable: false, width: 150 },
      {
      ...
      {
        text: "ตรวจสอบการเบิก/ย้ายกลุ่มตัดงบ",
        align: "center",
        sortable: false,
        width: 300,
      },
  ...
  created() {
    ...
    if (this.role.includes("ALL") || this.role.includes("COST")) {
      this.getdata();
      this.getdata_amount();
    } else {
  ...
  methods: {
    ...
    getdata_amount() {
      ...
      this.axios
        .get(
          process.env.VUE_APP_API + "/dashboard/document/amount/cost",
          headers
        )
        .then((response) => {
    ...
    getdata() {
      ...
      this.axios
        .get(
          process.env.VUE_APP_API + "/dashboard/document/cost?page=" +
            this.page + "&status=" +
            this.status + "&search=" +
            this.search,headers
        )
        .then((response) => {
          ...
          if (data.data) { //if แบบนี้คือเช็คอะไรคะ
            this.data = data.data;
          }
        })
        .catch((error) => {
    ...
    put_status(status, remark, uid) {
      ...
      this.axios
        .put(
          process.env.VUE_APP_API + "/dashboard/document/status/" + uid,
          data,
          headers
        )
        .then((response) => {
    ...
    select_transfer(uid) {
      this.$swal
        .fire({
          title: "ย้ายกลุ่มตัดงบ",
          input: "select",
          inputOptions: { COST: "COST", "SG&A": "SG&A", "R&D": "R&D" },
          inputPlaceholder: "กรุณาเลือกกลุ่มตัดงบที่ท่านต้องการโอนย้าย",
          confirmButtonText: "โอนย้าย",
          cancelButtonText: "ยกเลิก",
          showCancelButton: true,
          inputValidator: (value) => {
            if (!value) {
              return "กรุณาเลือกกลุ่มตัดงบที่ท่านต้องการโอนย้าย";
            }
          },
        })
        .then((result) => {
          if (result.isConfirmed) {
            this.put_transfer(result.value, uid);
          }
        });
    },
    put_transfer(cost_center, uid) {
      this.$swal.fire({
        title: "กำลังโหลด",
        allowEscapeKey: false,
        allowOutsideClick: false,
      ...
      this.axios
        .put(
          process.env.VUE_APP_API + "/dashboard/document/transfer/" + uid,
          data,
          headers
        )
        .then((response) => {
