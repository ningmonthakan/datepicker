      ...
      <div>
        <v-row>
          <v-col
            ...
            "
          >
            <h3 style="font-weight:700">ใบเสร็จ</h3>
          </v-col>
          <v-col cols="3">
...
export default {
  ...
  methods: {
    ...
    exportAll(){
      ...
      this.axios
        .get(process.env.VUE_APP_API + "/export/log/receipt",headers)
        .then((response) => {
            if(response.status == 200) {
              const fileURL = window.URL.createObjectURL(response.data);
              const fileLink = document.createElement("a");
              fileLink.href = fileURL;
              fileLink.setAttribute("download", "export_request_receipt_all.csv");
