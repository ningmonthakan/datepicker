      ...
      <div>
        <v-row>
          <v-col
            ...
            "
          >
            <h3 style="font-weight:700">Report หนังสือทวงหนี้</h3>
          </v-col>
          <v-col cols="3">
          ...
          <v-col cols="12">
            <v-card>
              ...
              <v-card-text>
                ...
                <!-- <download-csv :data="debt_data" name="export_report_debt_all.csv"
                  ><v-btn
                    color="teal"
                    dark
                    style="width: 150px; font-size: 12px; font-weight: 600"
                  ...
                  <div class="table-invoice" style="position: relative">
                    ...
                    >
                      ...
                      <template v-slot:[`item.start_date`]="{ item }">
                        <label v-if="parseDate(item.start_date) != '01/01/0001'">
                          {{ parseDate(item.start_date) }}</label
                        >
                      </template>
                      <template v-slot:[`item.end_date`]="{ item }">
                        <label v-if="parseDate(item.end_date) != '01/01/0001'">
                          {{ parseDate(item.end_date) }}</label
                        >
                      </template>
                      <template v-slot:[`item.due_date`]="{ item }">
                        <label v-if="parseDate(item.due_date) != '01/01/0001'">
                          {{ parseDate(item.due_date) }}</label
                        >
                      </template>
                      <!-- วันที่สิ้นสุดสัญญา -->
                      <template v-slot:[`item.so_end_date`]="{ item }">
                        <label v-if="parseDate(item.so_end_date) != '01/01/0001'">
                      ...
                      <template v-slot:[`item.memo_date`]="{ item }">
                        <label v-if="parseDate(item.memo_date) != '01/01/0001'">
                          {{ parseDate(item.memo_date) }}</label
                        >
                      </template>
                      <template v-slot:[`item.created_at`]="{ item }">
                        <label v-if="parseDate(item.created_at) != '01/01/0001'">
                          {{ parseDateRequest(item.created_at) }}</label
                        >
                      </template>
                    </v-data-table>
...
export default {
  ...
  methods: {
    ...
    Export_AllFile() {
      ...
      // };
      // this.axios
      //   .get(process.env.VUE_APP_API + "/debt/exportinvoicesdebtrpa", headers)
      //   .then((response) => {
      ...
      //         element.RequestDate = moment(element.RequestDate).format(
      //           "DD/MM/YYYY"
      //         );
      ...
      this.axios
        ...
        .then((response) => {
          if (response.data) {
            ...
            fileLink.setAttribute("download", "export_report_debt_all.csv");
      ...
      this.axios
        .get(
          process.env.VUE_APP_API +
            "/debt/searchloginvoicedebt?size=" +
            this.item_per_page +
            "&page=" +
            this.page +
            "&search=" +
